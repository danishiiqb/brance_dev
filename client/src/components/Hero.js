			...
			>
				<div className="absolute top-1/2 z-50 opacity-90 -translate-y-1/2 space-y-1 lg:space-y-2  left-1 lg:left-2.5">
					{[...Array(heroInfo.length)].map((_, index) => {
						return <div key={index} onClick={(_ => {
							setIdx(index)
						})} className={`w-1 h-1 lg:w-3 lg:h-3 cursor-pointer transition-all duration-500 ${idx === index ? 'bg-white' : 'bg-transparent'}  rounded-full border-[.5px] lg:border-2 border-black`}></div>
					})}
				</div>
				<div className="transform w-full lg:w-4/6 top-1/2 -translate-y-1/2 pl-4 lg:pl-11 relative">
						...
						{heroInfo[idx].heading.split("/").map((elem, idx) => {
							return (
								<div className="overflow-hidden">
									<span
										className={`head block transform translate-y-24 ${idx === heroInfo[idx].heading.split("/").length - 1
											? "text-black"
											: "text-white"
											} `}
									>
					...
					{width > 700 ?
						...
						</button> : <button className="bg-black shadow-dark text-[6px] text-white px-1 mt-2  rounded-sm font-bold py-1">Shop Now</button>}
				</div>
			</div>
		</>
	);
