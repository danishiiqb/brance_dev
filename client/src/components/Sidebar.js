...
function Sidebar({
	...
	useEffect(() => {
		if (width > 700) {
			...
			if (showInner)
				timeline.current
					.to(".slide", { width: "100vw", ease: "Power3.easeOut", duration: ".5" })
	...
	return (
		<div
			className={`absolute  slide top-0 transform -translate-x-full ${width > 700 && "flex"
				} 	 opacity-0  h-screen px-11  py-8 ${width > 700 ? " w-4/12 " : "w-full"
		...
		>
			...
			<Cross
				...
				className={`absolute ${width > 700 ? "top-8 w-6 h-6 right-10" : "w-4 h-4 top-9 right-8"
			...
			<div className="menu">
				...
				<div
					className={`mt-16 ${width < 700 && showInner ? " hidden" : "!visible"
						}`}
				>
			...
			<div
				className={`text-lg flex-1 ${width > 700 ? "ml-16 mt-20 " : " ml-8 mt-6 "
